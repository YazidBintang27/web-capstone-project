...
<body class="flex overflow-auto box-border font-[Open_Sans]">
    ...
    <main class="grow-5 mx-5">
        <h2 class="font-bold text-4xl text-gray-800 mb-2 mt-10">Manajemen Data Balita</h2>
        <div class="flex w-inherit justify-end mt-16 mx-5">
            <a href="{{ route('childs.create') }}"
               class="flex shadow-sm py-2 px-3 rounded-lg bg-green-400 items-center">
                ...
                <span class="ml-3 text-lg font-medium text-white">Tambah Data Balita</span>
            </a>
        </div>
        <x-table>
            <thead class="bg-[#33ccff]">
                <tr>
                    ...
                    <td class="border border-black px-5 py-1 text-lg text-gray-800 font-medium text-center">Tinggi Badan</td>
                    <td class="border border-black px-5 py-1 text-lg text-gray-800 font-medium text-center">Berat Badan</td>
                    <td class="border border-black px-5 py-1 text-lg text-gray-800 font-medium text-center">Jenis Kelamin</td>
                    <td class="border border-black px-5 py-1 text-lg text-gray-800 font-medium text-center">Status Gizi</td>
            ...
            <tbody>
                @forelse ($childs as $child)
                    <tr>
                        <td class="border px-5 py-1 text-center">{{ $child->name }}</td>
                        <td class="border px-5 py-1 text-center">{{ \Carbon\Carbon::parse($child->birthdate)->format('d-m-Y') }}</td>
                        <td class="border px-5 py-1 text-center">{{ $child->height }} cm</td>
                        <td class="border px-5 py-1 text-center">{{ $child->weight }} kg</td>
                        <td class="border px-5 py-1 text-center">
                            {{ $child->gender === 'L' ? 'Laki-Laki' : 'Perempuan' }}
                        </td>
                        <td class="border px-5 py-1 text-center">{{ $child->nutritional_status }}</td>
                        <td class="border px-5 py-1 text-center">
                            {{ $child->mother ? $child->mother->name : '-' }}
                        </td>
                        <td class="border px-5 py-1 text-center">
                            <div class="flex justify-center my-1">
                                ...
                                <form action="{{ route('childs.destroy', $child->id) }}" method="POST" class="ml-4">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
