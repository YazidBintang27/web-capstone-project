...
<body class="flex overflow-auto box-border font-[Open_Sans]">
    ...
    <main class="grow-5 mx-7.5">
        <h2 class="font-bold text-4xl text-gray-800 mb-2 mt-10">Edit Data Balita</h2>
        ...
        <form action="{{ route('childs.update', $child->id) }}" method="POST" class="w-inherit mt-10">
            @csrf
            @method('PUT')
            <fieldset class="w-inherit flex gap-10">
                <fieldset class="grow-1">
                    <label for="name" class="font-medium text-xs text-black">Nama Balita</label>
            ...
            <fieldset class="w-inherit flex mt-5 gap-5">
                <fieldset class="grow-1">
                    ...
                    <input type="date" name="birthdate" id="birthdate"
                        value="{{ old('birthdate', $child->birthdate) }}"
            ...
            <fieldset class="mt-5">
                ...
                <select name="nutritional_status" id="nutritional_status"
                    ...
                    <option value="Gizi Baik" {{ old('nutritional_status', $child->nutritional_status) == 'Gizi Baik' ? 'selected' : '' }}>Gizi Baik</option>
                    <option value="Gizi Kurang" {{ old('nutritional_status', $child->nutritional_status) == 'Gizi Kurang' ? 'selected' : '' }}>Gizi Kurang</option>
                    <option value="Gizi Buruk" {{ old('nutritional_status', $child->nutritional_status) == 'Gizi Buruk' ? 'selected' : '' }}>Gizi Buruk</option>
                    <option value="Gizi Lebih" {{ old('nutritional_status', $child->nutritional_status) == 'Gizi Lebih' ? 'selected' : '' }}>Gizi Lebih</option>
                </select>
            </fieldset>
            <fieldset class="mt-5">
            ...
            <div class="inline-flex gap-5">
                <button type="submit"
                    class="group inline-flex mt-8.5 items-center justify-center whitespace-nowrap rounded-lg py-2 align-middle text-sm font-semibold transition-all duration-300 ease-in-out bg-blue-700 px-6 text-white hover:bg-blue-950 h-[38px] min-w-[38px] gap-2">
