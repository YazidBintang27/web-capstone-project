...
<body class="flex overflow-auto box-border font-[Open_Sans]">
    ...
    <main class="grow-5 mx-5">
        <h2 class="font-bold text-4xl text-gray-800 mb-2 mt-10">Data Riwayat Imunisasi Balita</h2>
        <div class="flex w-inherit justify-end mt-16 mx-5">
            <a href="{{ route('immunizations.create') }}"
               class="flex shadow-sm py-2 px-3 rounded-lg bg-green-400 items-center">
                ...
                <span class="ml-3 text-lg font-medium text-white">Tambah Data Imunisasi</span>
            </a>
        </div>
        <x-table>
            <thead class="bg-[#33ccff]">
                <tr>
                    ...
                    <td class="border border-black px-5 py-1 text-lg text-gray-800 font-medium text-center">Jenis Vaksin</td>
                    <td class="border border-black px-5 py-1 text-lg text-gray-800 font-medium text-center">Tanggal Imunisasi</td>
    ...
    @forelse ($immunizations as $immunization)
        <tr>
            <td class="border px-5 py-1 text-center">
                {{ $immunization->child->name ?? '-' }}
            </td>
            <td class="border px-5 py-1 text-center">
                {{ $immunization->vaksin_type }}
            </td>
            <td class="border px-5 py-1 text-center">
                {{ \Carbon\Carbon::parse($immunization->immunization_date)->format('d-m-Y') }}
            </td>

            <td class="border px-5 py-1 text-center">
                <div class="flex justify-center my-1">
                    <a href="{{ route('immunizations.edit', $immunization->id) }}"
                       class="bg-yellow-400 font-medium text-center px-6 py-2 rounded-lg text-white flex items-center">
                        <img src="{{ asset('images/icon/Edit.png') }}" alt="edit" class="w-5">
                        <span class="ml-2">Edit</span>
                    </a>

                    <form action="{{ route('immunizations.destroy', $immunization->id) }}" method="POST" class="ml-4">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
                                onclick="return confirm('Yakin ingin menghapus data ini?')"
                                class="bg-red-400 font-medium text-center px-6 py-2 rounded-lg text-white flex items-center">
                            <img src="{{ asset('images/icon/Trash.png') }}" alt="delete" class="w-5">
    ...
    @empty
        <tr>
            <td colspan="4" class="border px-5 py-2 text-center text-gray-500 italic">
