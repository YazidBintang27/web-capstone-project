...
<body class="flex overflow-auto box-border font-[Open_Sans]">
    ...
    <main class="grow-5 mx-7.5">
        <h2 class="font-bold text-4xl text-gray-800 mb-2 mt-10">Edit Data Imunisasi</h2>
        ...
        <form method="POST" action="{{ route('immunizations.update', $immunization->id) }}" class="w-inherit mt-10">
            @csrf
            @method('PUT')
                        ...
                        <select name="child_id" id="child_id" required
                            ...
                            @foreach ($childs as $child)
                                <option value="{{ $child->id }}"
                                    {{ $child->id == $immunization->child_id ? 'selected' : '' }}>
                                    {{ $child->name }} ({{ $child->mother->name }})
                                </option>
                            @endforeach
                        </select>
                        @error('child_id')
                ...
                <fieldset class="grow-1">
                    ...
                    <div class="relative w-full">
                        <input type="date" name="immunization_date"
                               value="{{ old('immunization_date', $immunization->immunization_date) }}" required
                               class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm font-medium text-gray-700 focus:border-blue-600 focus:ring-1 focus:ring-blue-600">
            ...
            <fieldset class="mt-5">
                ...
                <div class="relative w-full">
                    <input type="text" name="vaksin_type"
                           value="{{ old('vaksin_type', $immunization->vaksin_type) }}"
