...
<body class="flex overflow-auto box-border font-[Open_Sans]">
    ...
    <main class="grow-5 mx-7.5">
        <h2 class="font-bold text-4xl text-gray-800 mb-2 mt-10">Tambah Data User</h2>
        ...
        <form method="POST" action="{{ route('users.store') }}" enctype="multipart/form-data" class="w-inherit mt-10">
            @csrf
            <fieldset class="w-inherit flex gap-10">
                ...
                <fieldset class="grow-1">
                    ...
                    <input type="text" name="username" id="username" value="{{ old('username') }}" placeholder="Masukan Username..."
                        class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm font-medium placeholder-slate-400 focus:border-blue-600">
                </fieldset>
            </fieldset>
            <fieldset class="w-inherit flex mt-5 gap-10">
                <fieldset class="grow-1">
                    ...
                    <input type="text" name="nik" id="nik" value="{{ old('nik') }}" placeholder="Masukan NIK..."
                        class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm font-medium placeholder-slate-400 focus:border-blue-600">
                </fieldset>

                <fieldset class="grow-1">
                    <label class="font-medium text-xs text-black" for="phone_number">Nomor Telepon</label>
                    <input type="text" name="phone_number" id="phone_number" value="{{ old('phone_number') }}" placeholder="Masukan Nomor Telepon..."
                        class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm font-medium placeholder-slate-400 focus:border-blue-600">
                </fieldset>
            </fieldset>
            <fieldset class="inline-flex flex-col items-start gap-1.5 w-full mt-5">
                ...
                <textarea name="address" id="address" rows="5" placeholder="Masukan Alamat..."
                    class="w-full rounded-lg border border-slate-200 p-3 text-sm placeholder-slate-500 focus:border-blue-600">{{ old('address') }}</textarea>
            </fieldset>
            <fieldset class="w-inherit flex mt-5 gap-10">
                <fieldset class="grow-1">
                    <label class="font-medium text-xs text-black" for="password">Password</label>
            ...
            <fieldset class="w-inherit mt-5">
                ...
                <select name="role" id="role"
                    ...
                    @if ($currentRole === 'admin')
                        ...
                        <option value="kepala_posyandu" {{ old('role') == 'kepala_posyandu' ? 'selected' : '' }}>Kepala Posyandu</option>
                        <option value="kader" {{ old('role') == 'kader' ? 'selected' : '' }}>Kader</option>

                    @elseif ($currentRole === 'kepala_posyandu')
                        <option value="kader" {{ old('role') == 'kader' ? 'selected' : '' }}>Kader</option>

                    @elseif ($currentRole === 'kader')
                        <option disabled>Tidak dapat menambah pengguna lain</option>
                    @endif
                </select>
